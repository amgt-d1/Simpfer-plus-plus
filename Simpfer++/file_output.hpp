...
std::vector<double> block_access_vec;

// result output
void output_result_pre()
{
    // file-name
	std::string f_name = "result/" + std::to_string(dataset_id) + "-";
	if (dataset_id == 0) f_name += "MovieLens/";
	if (dataset_id == 1) f_name += "Netflix/";
	if (dataset_id == 2) f_name += "Yahoo/";
	if (dataset_id == 3) f_name += "Amazon/";

	f_name += "index_id(" + std::to_string(dataset_id) + ")_dim(" + std::to_string(dimensionality) + ")_sampling-rate(" + std::to_string(sampling_rate) + ")_thread(" + std::to_string(thread_num) + ").csv";
...
void output_result()
{
	...
	f_name += "simpfer_id(" + std::to_string(dataset_id) + ")_dim(" + std::to_string(dimensionality) + ")_sampling-rate(" + std::to_string(sampling_rate) + ")_sampling-rate-user(" + std::to_string(sampling_rate_user) + ")_k(" + std::to_string(k) + ").csv";
	...
	file << "processing time [msec]" << "," << "#ip comp." << "," << "#results" << "," << "#cores" << "," << "c" <<"\n";
	file << time_online_processing / (1000 * query_item_cardinality) << "," << ip_count / query_item_cardinality << "," << result_count / query_item_cardinality << "," << thread_num << "," << c << "\n\n";
}