...
public:
	...
	void make_matrix()
	{
		M = Eigen::MatrixXf::Zero(member.size(), dimensionality);
		for (unsigned int i = 0; i < member.size(); ++i)
		{
			for (unsigned int j = 0; j < dimensionality; ++j) M(i,j) = member[i]->vec[j];
		}
	}
};
...
void input_matrix()
{
	...
	{
		if (dataset_id == 0) f_name = "/mnt/d/_dataset/MatrixFactorization/movielens_mf-50.txt";
		if (dataset_id == 1) f_name = "/mnt/d/_dataset/MatrixFactorization/netflix_mf-50.txt";
		if (dataset_id == 2) f_name = "/mnt/d/_dataset/MatrixFactorization/yahoosong_mf-50.txt";
		if (dataset_id == 3) f_name = "/mnt/d/_dataset/MatrixFactorization/amazon_mf-50.txt";
		...
		while (std::getline(ifs_file, full_data))
		{
			...
			if (flag == "T")
			{
				if (type[0] == 'p') {
					...
					if (uni_rnd(mt) <= sampling_rate_user)
					{
